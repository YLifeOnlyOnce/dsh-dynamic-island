...
`dsh-dynamic-island` 是一个面向 DeepSeek Harness 的高保真交互原型：用 Apple Liquid Glass 风格的灵动岛，把 Agent 的思考、工具执行、审批、异常和完成状态变成轻量、可读、有一点陪伴感的工作界面。

这不是一只与任务无关的桌宠。角色的每一次形态变化都应该来自真实的 Harness 状态，用户始终能知道 Agent 是否在工作、正在做什么、是否需要确认以及任务结果如何。
